...
impl Config2 {
    fn load() -> Config2 {
        ...
        // 2. 默认拒绝局域网发现 (N 表示禁用发现 即拒绝)
        if !config.options.contains_key("enable-lan-discovery") {
            config.options.insert("enable-lan-discovery".to_owned(), "N".to_owned());
            store = true;
        }
        // 3. 默认一次性密码长度为 10
...
impl LocalConfig {
    fn load() -> LocalConfig {
        ...
        if !config.options.contains_key("enable-ipv6-punch") {
            config.options.insert("enable-ipv6-punch".to_owned(), "Y".to_owned());
            store = true;
        }
        // 3. 默认取消勾选"启动时检查软件更新"
        if !config.options.contains_key("enable-check-update") {
            config.options.insert("enable-check-update".to_owned(), "N".to_owned());
            store = true;
        }
